...
def _make_mock_client(return_value: BrokerOverview | None = None, side_effect: Exception | None = None) -> MagicMock:
    """Return a fake RabbitMQHttpClient instance."""
    instance = MagicMock()
    if side_effect is not None:
        instance.get_overview = AsyncMock(side_effect=side_effect)
    else:
        instance.get_overview = AsyncMock(return_value=return_value or _HEALTHY_OVERVIEW)
    return instance
...
def test_broker_overview_returns_200(client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
...
def test_broker_overview_checks_structure(client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "app.api.routes.observability.RabbitMQHttpClient",
        lambda: _make_mock_client(),
    )
...
def test_broker_overview_queues_structure(client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "app.api.routes.observability.RabbitMQHttpClient",
        lambda: _make_mock_client(),
    )
...
def test_broker_overview_empty_queues(client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_broker_overview_degraded_broker(client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
    overview = BrokerOverview(
    ...
    data = client.get("/observability/broker").json()
    # Route returns 200 even when broker reports degraded — caller interprets the payload
...
def test_broker_overview_503_on_connection_error(client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "app.api.routes.observability.RabbitMQHttpClient",
...
def test_broker_overview_503_on_timeout(client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
    import httpx
